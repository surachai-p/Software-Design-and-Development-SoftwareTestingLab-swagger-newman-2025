  ...
  item: [
    ...
    // ── Request 1: POST /api/login ───────────────────────────
    {
      ...
      event: [{ listen: 'test', script: { type: 'text/javascript', exec: [
        ...
        // เพิ่มการตรวจสอบ user.id ตรงนี้
    ...
    // ── Request 2: POST /api/bookings ────────────────────────
    {
      ...
      request: {
        ...
        body: {
          mode: 'raw',
          raw: JSON.stringify({
            fullname: 'Jakkit', email: '68030033@kmitl.ac.th',
    ...
    // ── Request 5: GET /api/bookings/:id ─────────────────────
    {
      ...
      event: [{ listen: 'test', script: { type: 'text/javascript', exec: [
        ...
        'pm.test("Verified by Student: [จักรกฤษณ์ บางต่าย]", function() {',
        '  const d = pm.response.json();',
        '  pm.expect(d.fullname).to.not.be.empty;',
        '  console.log("Testing booking for: " + d.fullname);',
        '});'
      ]}}],
      request: {
        method: 'GET',
    ...
    // ── Request 9: POST /api/checkin ───────────────────────────
{
  ...
  event: [{ listen: 'test', script: { type: 'text/javascript', exec: [
    // แก้ไขโดย: [จักรกฤษณ์  บางต่าย] รหัสนักศึกษา: [68030033]
    'pm.test("Status code is 200 - CheckIn Successful", function() {',
    '  pm.response.to.have.status(200);',
    '});',

    'pm.test("Response returns checkin details", function() {',
    '  const d = pm.response.json();',
    '  pm.expect(d).to.have.property("status", "checked-in");',
    '  pm.expect(d).to.have.property("checkin_time");',
    '  pm.expect(d.booking_id).to.eql(14);', // ตรวจสอบว่า ID ตรงกับที่ส่งไป
    '});',

    'pm.test("Verified by Student", function() {',
    '  console.log("Check-In API verified by: [จักรกฤษณ์ บางต่าย]");',
    '});'
  ]}}],
  request: {
    method: 'POST',
